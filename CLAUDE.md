...
- **Campos e onde nascem:**
  - `SourceReferrerURL` + UTMs — captura **por toque, consistente** (`site.js`): qualquer `utm_*` na URL conta como um novo toque de aquisição e **reconstrói o conjunto INTEIRO de UTMs de uma vez** (`fei_utms` no localStorage; params ausentes na URL viram vazio) — nunca mescla campos de campanhas diferentes (ex.: um toque novo do Google não mantém mais um `utm_source=chatgpt.com` de um toque anterior). No mesmo toque o **referrer é atualizado junto** (só se `document.referrer` não estiver vazio, pra não apagar um referrer bom com um toque sem referrer). **Sem UTMs na URL** (navegação interna): as UTMs ficam como estão e o `referrer` (`fei_referrer`) é gravado só no primeiro acesso e nunca sobrescrito depois — senão perderia o referrer externo original assim que `document.referrer` vira o próprio site.
