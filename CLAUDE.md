...
- Webhook: `https://flow.fei.edu/webhook/lead-conversion` (POST JSON, endpoint de produção). Ligado ao form real do site (`astro/public/site.js`, `LEAD_WEBHOOK_URL`) — modal Apply e página `/apply` fazem POST de verdade aqui. **Contrato do payload em `docs/lead-webhook-payload.md`**. Não usar o caminho `/webhook-test/`: ele só responde com o editor do n8n "escutando" e aceita 1 request.
...
- **Campos e onde nascem:**
  - `SourceReferrerURL` + UTMs + **click IDs (`gclid`/`fbclid`)** — captura **por toque, consistente** (`site.js`, constante `TOUCH_KEYS` = 6 UTMs + 2 click IDs): qualquer um dos 8 params na URL conta como um novo toque de aquisição e **reconstrói o conjunto INTEIRO de uma vez** (`fei_utms` no localStorage; params ausentes na URL viram vazio) — nunca mescla campos de campanhas diferentes (ex.: um toque novo do Google não mantém mais um `utm_source=chatgpt.com` de um toque anterior). No mesmo toque o **referrer é atualizado junto** (só se `document.referrer` não estiver vazio, pra não apagar um referrer bom com um toque sem referrer). **Sem UTMs na URL** (navegação interna): as UTMs ficam como estão e o `referrer` (`fei_referrer`) é gravado só no primeiro acesso e nunca sobrescrito depois — senão perderia o referrer externo original assim que `document.referrer` vira o próprio site.
